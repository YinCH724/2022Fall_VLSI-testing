...
#include <stdlib.h>
extern int ASS3_counter;
extern int ASS3_patternNum;
...
class CIRCUIT
{
    private:
        ...
        list<FAULT*> UCPFlist;
       	...
       	list<Bridging_FAULT*> BFlist;
        list<Bridging_FAULT*> UBFlist;
	...
	//defined in ass2.cc
	VALUE Evaluate_CPU(GATEPTR gptr);
	void LogicSimVectors_CPU();
	void LogicSim_CPU();

	void CreateLogicSim();
	void WriteParallelIOs(unsigned idx, ofstream &File);
	
	//definded in ass5.cc
	void BridgeFaultSimVectors();
	void BridgeFaultSim();
	void InjectBFaultValue(GATEPTR gptr1, GATEPTR gptr2, unsigned idx, BRIDGING bridge);
	...
	void Faultlist_ASS6();
	void Atpg_ASS6();
	double Fault_Coverage_ASS6(ofstream &File);
	
	GATEPTR Set_Bcondition_01(Bridging_FAULT* fptr);
	GATEPTR Set_Bcondition_10(Bridging_FAULT* fptr);
	void Atpg_bridge();
	ATPG_STATUS Bridging_Podem(Bridging_FAULT* fptr, unsigned &total_backtrack_num);
	ATPG_STATUS Bridging_SetUniqueImpliedValue(Bridging_FAULT* fptr, bool type);
	bool Bridging_FaultEvaluate(Bridging_FAULT* fptr);
};
#endif
