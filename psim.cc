...
void CIRCUIT::ParallelLogicSimVectors()
{
	...
	OutputFile.append("_plogicsim.out");
	...
	ScheduleAllPIs();
	ParallelLogicSim();
	WriteParallelIOs(pattern_idx, File);
	//PrintParallelIOs(pattern_idx);
    }
    ASS3_patternNum=pattern_num;
    File.close();
}
...
void CIRCUIT::WriteParallelIOs(unsigned idx, ofstream &File)
{
