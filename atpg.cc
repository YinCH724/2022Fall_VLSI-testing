...
void CIRCUIT::PrintFaultList()
{
	GATE* gptr;
	FAULT *fptr;
	list<FAULT*>::iterator fite;
	for (fite = CPFlist.begin(); fite != CPFlist.end();++fite) {
        fptr = *fite;
		...
		cout<<gptr->GetName()<<" s.a. "<<fptr->GetValue()<<endl;
	}
}
void CIRCUIT::CompareFaultList(){
	GenerateAllFaultList();
	GenerateCheckPointFaultList();
	int orgNum1 = CPFlist.size();
	cout<<endl<<"Number of Faultlist by CheckPoint Theorem: "<<orgNum1<<endl;
	int orgNum2 = Flist.size();
	cout<<"Number of All Faultlist: "<<orgNum2<<endl;
	int CollapseNum = orgNum2-orgNum1;
	cout<<"Percentage of Fault Had Been Collapsed: "<<100*float(CollapseNum)/float(orgNum2)<<"%"<<endl<<endl;
}

void CIRCUIT::GenerateBridgingFaultList(){
	Bridging_FAULT* Bfptr;
	SetMaxLevel();
	vector<GATE*> Lastwire(MaxLevel);
	unsigned level;
	for(unsigned i = 0;i<Netlist.size();++i){
		level = Netlist[i]->GetLevel();
		if (Lastwire[level]!=NULL){
			if (level!=MaxLevel){
				Bfptr = new Bridging_FAULT(Lastwire[level], Netlist[i], AND, level);
				BFlist.push_front(Bfptr);
				Bfptr = new Bridging_FAULT(Lastwire[level], Netlist[i], OR, level);
				BFlist.push_front(Bfptr);
			}
		}
		Lastwire[level] = Netlist[i];
	}
	UBFlist = BFlist;
	/*
	if (option.retrieve("output")){
		...
		for (fite = BFlist.begin(); fite != BFlist.end();++fite) {
        	...
        	Bfptr = *fite;	
        	//file<<Bfptr->GetLevel()<<endl;
			gptr = Bfptr->GetNet1();
			file<<"("<<gptr->GetName()<<", ";
			gptr = Bfptr->GetNet2();
			file<<gptr->GetName()<<", ";
			if(Bfptr->GetBridging()==AND)
				file<<"AND"<<")"<<endl;
			else if(Bfptr->GetBridging()==OR)
				file<<"OR"<<")"<<endl;
...
void CIRCUIT::Atpg()
{
    ...
    for (fite = Flist.begin(); fite != Flist.end();++fite) {
        ...
        /*
        	cout<<endl<<"PODEM: "<<fptr->GetInputGate()->GetName()<<" "<<fptr->GetOutputGate()->GetName()<<" s.a. "<<fptr->GetValue()<<" have detected!"<<endl;*/
        	
        	continue; 
        }
        //run podem algorithm
        /*
        cout<<endl<<"Faults "<<fptr->GetInputGate()->GetName()<<" "<<fptr->GetOutputGate()->GetName()<<" s.a. "<<fptr->GetValue()<<endl;
        */
...
ATPG_STATUS CIRCUIT::Podem(FAULT* fptr, unsigned &total_backtrack_num)
{
    ...
    while(backtrack_num < BackTrackLimit && status == FALSE) {
        ...
        if (pi_gptr) { //decision found
            ...
            /*
            cout<<"Backtrace: PI! "<<decision_gptr->GetName()<<endl;
            */
        }
            ...
            while (!GateStack.empty() && !pi_gptr) {
                ...
                if (decision_gptr->GetFlag(ALL_ASSIGNED)) {
                    ...
                    /*
                    cout<<decision_gptr->GetName()<<" all possible value have been assigned!"<<endl;
                    */
                ...
                //inverse current decision value
                    ...
                    /*
                    cout<<"Backtracking: "<<decision_gptr->GetName()<<endl;
                    cout<<"Decision: PI "<<decision_gptr->GetName()<<" "<<decision_gptr->GetValue()<<endl;
                    */
                    ScheduleFanout(decision_gptr);
        ...
        if (pi_gptr) {
            ...
            if (CheckTest()) { 
            	//cout<<"Fault propogate success (3 or 4 propogate to PO)"<<endl;
            	status = TRUE;
            }
        }
    } //end while loop
    ...
    /*
    if (status ==  TRUE) {
    	cout<<endl<<"pattern found!"<<endl;
		for (i = 0;i<PIlist.size();++i) {
		    cout<<PIlist[i]->GetName()<<" "<<PIlist[i]->GetValue()<<endl;
		}//end for all PI
		cout<<endl;
    } //end status == TRUE
	*/
...
GATEPTR CIRCUIT::TestPossible(FAULT* fptr)
{
    ...
    if (!ogptr->GetFlag(OUTPUT)) {
        if (ogptr->GetValue() != X) {
            ...
            /*
            if (decision_gptr){
            	cout<<"Fault propogation(D-frontier): ";
            	cout<<decision_gptr->GetName()<<endl;
            }
            */
            ...
            //cout<<"Fault activation: "<<fptr->GetInputGate()->GetName()<<endl;
...
GATEPTR CIRCUIT::FindPIAssignment(GATEPTR gptr, VALUE value)
{
    ...
    if (gptr->GetFunction() == G_PI) {
        gptr->SetValue(value);
        /////////////
        //cout<<"Backtrace: PI! "<<gptr->GetName()<<" assign "<<value<<endl;
        return gptr;
    }
    ...
    /*
    if (j_gptr){
    	cout<<"Backtrace: Gate "<<gptr->GetName()<<" "<<value<<endl;}*/
    	
    if (j_gptr) { return FindPIAssignment(j_gptr, j_value); }
    //cout<<"This path can not backtrace to PI"<<endl;
    return 0;
}
...
bool CIRCUIT::CheckTest()
{
    ...
    for (unsigned i = 0;i<POlist.size();++i) {
        ...
        cout<<POlist[i]->GetName()<<" "<<value<<endl;
    }
    cout<<endl;
    */
    for (unsigned i = 0;i<POlist.size();++i) {
    ...
    for (gite = PropagateTree.begin();gite!=PropagateTree.end();++gite) {
        ...
        for (i = 0;i<gptr->No_Fanin(); ++i) {
            ...
            if (TraceUnknownPath(gptr)) { 
            	//cout<<"D-frontier: "<<gptr->GetName()<<endl;
            	return gptr; 
            }
            break;
        }
